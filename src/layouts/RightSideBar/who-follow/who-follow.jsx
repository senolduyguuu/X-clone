...
const WhoFollow = () => {
	return (
		<SidebarSection title="Kimi Takip etmeli" more="/deneme">
			{
				// eslint-disable-next-line no-unused-vars
				whoFollowUsers.map((user, index) => {
					return (
						<button key={index} className="py-3 px-4 flex gap-3 transition-colors hover:bg-white/[0.03]">
							<img src={user.avatar} className="w-10 h-10 rounded-full object-cover" alt=""></img>
							<div className="flex-1 flex flex-col text-left">
								<div className="text-[15px] text-[#e7e9ea] leading-5 font-bold flex items-center">
									{user.fullName}
									{user?.verified && (
										<>
											<svg viewBox="0 0 22 22" width={18.75} height={18.75} aria-label="Verified account" role="img" className="text-[#1d9bf0] ml-05">
												<g>
